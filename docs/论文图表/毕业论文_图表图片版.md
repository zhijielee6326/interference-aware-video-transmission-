...
(2) A complete adaptive video transmission system is designed and implemented, covering the full pipeline from video capture, JPEG encoding, custom framing, FEC encoding, QPSK modulation to channel transmission, AGC, CFO correction, demodulation, decoding, and video display. Based on CCNN-detected interference type and confidence, the system dynamically adjusts JPEG encoding quality (15-85), FEC redundancy (1-3x), and video frame rate (5-25fps) through a multi-level hysteresis counter mechanism.
...
2. **训练数据生成与Python信号生成器开发**：初期基于MATLAB实现了6种干扰信号的生成器（16个脚本，1457行代码），后期使用Python精确复现MATLAB逻辑并整合为1个文件（465行），扩展为7类、宽干信比（-2~16 dB）训练数据集，共13,300个样本。Python方案实现了按需在线生成，与系统仿真信道无缝集成。
...
![表2-1 USRP B210主要参数](表格图片/01_表2-1 USRP B210主要参数.png)
...
多头注意力机制允许模型同时关注输入序列的不同位置和不同表示子空间，由Vaswani等人[12]在Transformer架构中首次提出。因果注意力（Causal Attention）确保每个位置的注意力计算只依赖于当前位置及之前的位置，符合时序信号因果关系。Van Den Oord等人[22]提出的WaveNet首次将因果卷积应用于序列建模，本系统在TCC模块中借鉴了因果卷积的设计思想。
...
![图3-1 系统总体架构图](image2插图/图3-1_系统总体架构图_image2.png)
...
![图3-2 仿真模式与USRP实机模式对比图](image2插图/图3-2_仿真模式与USRP实机模式对比图_image2.png)

*图3-2 仿真模式与USRP实机模式对比图*

![表3-1 系统运行模式对比](表格图片/02_表3-1 系统运行模式对比.png)
...
![表3-2 系统参数配置](表格图片/03_表3-2 系统参数配置.png)
...
![图4-2 CCNN网络结构图](image2插图/图4-2_CCNN网络结构图_image2.png)
...
![表4-1 训练数据集构成](表格图片/04_表4-1 训练数据集构成.png)
...
![表4-2 训练超参数配置](表格图片/05_表4-2 训练超参数配置.png)
...
![表4-3 不同JSR下的识别准确率（%）](表格图片/06_表4-3 不同JSR下的识别准确率_%.png)

*表4-3 不同JSR下的识别准确率（%）*

![图4-4 JSR vs 识别准确率曲线](gnuradio/jsr_sweep_results.png)
...
![表4-4 MATLAB与Python数据生成对比](表格图片/07_表4-4 MATLAB与Python数据生成对比.png)
...
![表5-1 数据包字段说明](表格图片/08_表5-1 数据包字段说明.png)
...
上述五类特征共同构成多维特征向量，在系统中承担辅助角色。本系统采用**CCNN 7类直接识别方案**：由于CCNN的7类输出中包含"无干扰（Clean）"类，模型在一次前向推理中同时完成"是否存在干扰"的检测和"干扰类型"的识别，无需单独的二值检测模块。PSD特征提取模块的主要作用为：（1）为实时可视化面板提供频谱显示数据；（2）当CCNN模型不可用时，作为降级检测方案，通过peak_to_noise和norm_entropy等特征进行粗略的干扰判定。相比两阶段级联方案（先检测再识别），7类直接识别避免了检测→识别之间的级联误差，且推理开销仅一次前向传播，更为简洁高效。
...
![表5-2 干扰等级划分与传输参数调整](表格图片/09_表5-2 干扰等级划分与传输参数调整.png)
...
![图5-2 自适应传输策略闭环图](image2插图/图5-2_自适应传输策略闭环图_image2.png)
...
![表6-1 实验环境](表格图片/10_表6-1 实验环境.png)
...
![表6-2 各类别分类性能](表格图片/11_表6-2 各类别分类性能.png)

*表6-2 各类别分类性能*

仅有个别样本被误分类，总体准确率达到99.91%。

![图6-1 七类信号混淆矩阵](gnuradio/figure_4_5_confusion_matrix.png)
...
![表6-3 CCNN模型各模块参数量分析](表格图片/12_表6-3 CCNN模型各模块参数量分析.png)
...
![表6-4 不同方法在7类干扰识别任务上的性能对比](表格图片/13_表6-4 不同方法在7类干扰识别任务上的性能对比.png)
...
![表6-5 仿真模式无干扰测试](表格图片/14_表6-5 仿真模式无干扰测试.png)
...
![表6-6 自动演示测试序列](表格图片/15_表6-6 自动演示测试序列.png)
...
![表6-7 USRP实机模式120秒测试结果](表格图片/16_表6-7 USRP实机模式120秒测试结果.png)

*表6-7 USRP实机模式120秒测试结果*

USRP实机模式下，TX和RX链路正常工作。需要说明以下几点：

1. **帧数差异**：接收帧数（732）多于发送帧数（365），这是因为USRP接收机持续采集射频数据流，接收端的帧同步模块在数据流中搜索帧头（0xAA55AA55同步字），可能将噪声或残余信号中的伪同步模式误判为有效帧起始，导致统计的"接收帧数"偏高。实际通过CRC-16校验的有效帧比例较低。
2. **实机解码率**：由于USRP在Python环境下存在射频频偏（CFO）、采样时偏、以及USB传输缓冲区溢出（overrun）等因素，实际帧解码成功率受到一定限制。未来可通过优化CFO估计算法、增大接收缓冲区、或采用FPGA级预处理来提升解码率。
3. **干扰识别有效性**：CCNN在实机环境下正确识别出MTJ、NAM、STJ、NFM等多种干扰类型，说明仿真训练的模型具有良好的仿真到实机（sim-to-real）迁移能力。

#### 6.5.3 可视化测试

使用`run_system_with_viz.py --mode full`启动USRP可视化模式，15秒测试结果：

![表6-8 USRP实机可视化长测结果](表格图片/17_表6-8 USRP实机可视化长测结果.png)

*表6-8 USRP实机可视化长测结果*

4个可视化子图均正常工作，实时频谱图显示RF信号功率谱，CCNN概率柱状图实时更新，时域波形显示IQ信号，JSR曲线静态展示模型性能。

![图6-2 USRP实机测试界面](gnuradio/usrp_test_report.png)

### 6.6 自适应策略有效性验证

通过仿真模式注入不同类型干扰，观察自适应策略的切换行为：

![表6-9 自适应策略有效性验证](表格图片/18_表6-9 自适应策略有效性验证.png)

*表6-9 自适应策略有效性验证*

自适应策略能够根据干扰类型正确切换传输参数，干扰清除后恢复至正常传输模式。

![图6-3 自适应策略有效性对比](gnuradio/fig_adaptive_comparison.png)
...
值此毕业之际，衷心感谢导师刘磊老师在毕业设计全过程中给予的悉心指导。从课题方向的选定、技术方案的设计论证，到系统调试过程中遇到的关键问题，刘老师始终以专业的视角给予耐心的指导和建议。在USRP硬件调试和实验方案设计阶段，刘老师的指导帮助我少走了许多弯路，使本项目得以顺利完成。

感谢实验室各位同学在系统联调、USRP设备借用和实验过程中提供的帮助与支持，与大家的讨论和交流使我对通信系统和深度学习有了更深入的理解。

感谢北京信息科技大学信息与通信工程学院四年来的培养，各位授课老师在通信原理、数字信号处理、电磁场与电磁波等课程中打下的专业基础，是完成本毕业设计的重要知识储备。

最后，感谢家人在大学四年中给予的无条件理解和支持，是你们的鼓励让我能够专注于学业，顺利完成本科阶段的学习。
