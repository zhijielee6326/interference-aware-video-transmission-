...
│   │   ├── TX_LFM/MTJ/NAM/NFM/STJ/SIN.m    # 6类干扰信号生成
│   │   ├── PSK_MOD.m                        # PSK调制
│   │   ├── study.m                          # 批量生成主脚本
│   │   ├── data_process.py                  # .mat → .npz 格式转换
│   │   └── data/                            # 3600个 .mat 文件
│   ├── generate_training_data.py            # Python版批量生成宽JSR训练数据
│   ├── usrp_model_test_v4.py                # Python信号生成器 (替代MATLAB)
│   └── datasets/                            # 生成后的数据集
│       ├── train/                           # 旧6类训练集 (39 npz)
│       ├── train_wide_jsr/                  # 7类宽JSR训练集 (best)
│       ├── test/                            # 测试集 (7 npz)
│       ├── samples/                         # 演示样本 (6 npz)
│       └── raw/                             # 原始数据
│
...
│   ├── models/                              # 训练好的模型权重
│   │   ├── wide_jsr/                        # 7类模型 (best: epoch_26 99.91%)
│   │   ├── best/                            # 旧6类最佳 (99.13%)
│   │   ├── archive/                         # ONNX / .om 部署格式
│   │   └── legacy/                          # 早期实验模型
│   ├── scripts/
│   │   ├── training/CCNN.py                 # 模型定义 + 训练代码
│   │   ├── inference/Infer.py               # 华为昇腾NPU推理引擎
│   │   └── utils/                           # 分析/对比/验证工具
│   ├── results/                             # 训练日志与结果图
...
│   ├── complete_system.py                   # USRP B210 稳定收发系统
│   ├── run_demo.sh                          # 一键启动脚本
│   ├── run_loopback_test.sh                 # USRP自发自收测试
│   ├── run_antenna_test.sh                  # 天线测试脚本
│   ├── usrp_chain_demo.py                   # USRP链路演示
│   ├── usrp_gnuradio_antenna_demo.py        # 天线频谱演示
│   ├── generate_training_data.py            # 训练数据生成 (副本)
│   └── README_*.md                          # 各模块说明文档
│
...
- **最佳模型**：`02_模型训练/models/wide_jsr/ccnn_epoch_26_acc_0.9991.pth`
...
../venv/bin/python3 cnn_interference_detector.py \
    --model ../02_模型训练/models/wide_jsr/ccnn_epoch_26_acc_0.9991.pth \
    --ccnn ../02_模型训练/scripts/training/CCNN.py --classes 7
```
